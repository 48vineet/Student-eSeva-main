...
const ActionApproval = ({ student, onClose }) => {
  ...
  const handleActionUpdate = async (actionId, status, rejectionReason = '') => {
    setLoading(true);
    try {
      await axios.put(`/api/students/${student.student_id}/actions/${actionId}`, {
        status,
        rejection_reason: rejectionReason
      });
      setMessage(`Action ${status} successfully`);
      fetchActions(); // Refresh actions
    } catch (error) {
      setMessage('Error updating action: ' + (error.response?.data?.error || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white">
        <div className="mt-3">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h3 className="text-lg font-medium text-gray-900">Action Approval</h3>
                ...
                <div key={action._id} className="border rounded-lg p-4 bg-white shadow-sm">
                  ...
                  {action.status === 'pending' && (
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleActionUpdate(action._id, 'approved')}
                        disabled={loading}
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (reason) {
                            handleActionUpdate(action._id, 'rejected', reason);
                          }
                        }}
                        disabled={loading}
