...
class RiskCalculator {
  ...
  async calculateRisk(data) {
    ...
    console.log(
      `🔍 Risk Calculator - Starting calculation for ${data.student_id}:`,
      {
    ...
    // Attendance rules
    console.log(
      `🔍 Risk Calculator - Attendance analysis for ${data.student_id}:`,
      {
        attendance_rate: data.attendance_rate,
    ...
    console.log(
      `🔍 Risk Calculator - Fee status analysis for ${data.student_id}:`,
      {
        fees_status: data.fees_status,
        fee_status: data.fee_status,
        feeStatus,
    ...
    // Build ML features from the same student snapshot and blend with rules.
    const mlFeatures = this.buildMlFeatures(data);
    let mlPrediction = null;
    try {
      mlPrediction = await predictRisk(mlFeatures);
    } catch (error) {
      console.error(
    ...
    const mlScore = Number(mlPrediction?.risk_score || 0);
    const hasUsableMl = Number.isFinite(mlScore) && mlScore >= 0;

    // Hybrid score: rules stay primary, ML acts as calibrated secondary signal.
    const blendedScore = hasUsableMl
      ? Math.min(100, Math.max(0, score * 0.7 + mlScore * 0.3))
      : Math.min(score, 100);

    const level =
      blendedScore >= 60 ? "high" : blendedScore >= 30 ? "medium" : "low";

    if (hasUsableMl) {
      factors.push("hybrid_ml_blend");
      explanation.push(
        `Hybrid model score: rules ${Math.round(score)}, ML ${Math.round(mlScore)}`,
      );
    }

    const result = {
      risk_level: level,
      risk_score: Math.round(blendedScore * 100) / 100,
  ...
  analyzeAcademicPerformance(data) {
    ...
    console.log(
      `🔍 Risk Calculator - Analyzing grades for ${data.student_id}:`,
      {
    ...
    if (failingCount >= this.thresholds.failingSubjectsHigh) {
      ...
      explanations.push(
        `Failing ${failingCount} out of ${totalSubjects} subjects`,
      );
  ...
  analyzeGradeProgression(data) {
    ...
    const hasDetailedGrades =
      data.unit_test_1_grades &&
      data.unit_test_2_grades &&
      data.mid_sem_grades &&
      data.end_sem_grades;
    ...
    if (!hasDetailedGrades) {
      ...
      if (avgGrade < 50) {
        ...
        recommendations.push({
          action: "Intensive academic support",
          urgency: "high",
        });
      } else if (avgGrade < 70) {
        score += 10;
    ...
    subjects.forEach((subject) => {
      ...
      if (progression.trend === "declining") {
        ...
        explanations.push(
          `${subject}: Declining from ${progression.start} to ${progression.end} (${progression.decline}% drop)`,
        );
    ...
    if (decliningSubjects > 0) {
      ...
      if (declineRatio >= 0.5) {
        // 50% or more subjects declining
        score += 30;
        factors.push("severe_grade_decline");
        explanations.push(
          `${decliningSubjects} out of ${subjects.length} subjects showing declining performance`,
        );
        recommendations.push({
          action: "Immediate academic intervention required",
          urgency: "immediate",
        });
      } else if (declineRatio >= 0.25) {
        // 25% or more subjects declining
        score += 20;
  ...
  calculateSubjectProgression(data, subject) {
    const grades = [
      this.getSubjectGrade(data.unit_test_1_grades, subject, "unit_test_1"),
      this.getSubjectGrade(data.unit_test_2_grades, subject, "unit_test_2"),
      this.getSubjectGrade(data.mid_sem_grades, subject, "mid_sem"),
      this.getSubjectGrade(data.end_sem_grades, subject, "end_sem"),
   ...
   */
  getSubjectGrade(gradeSet, subject, examType) {
    if (!gradeSet) return null;

    const grade = gradeSet.find((g) => g.subject === subject);
    if (!grade) return null;

    // New detailed schema stores per-exam fields; keep score fallback for old data.
    if (examType && grade[examType] !== undefined && grade[examType] !== null) {
      return Number(grade[examType]);
    }

    if (grade.final_grade !== undefined && grade.final_grade !== null) {
      return Number(grade.final_grade);
    }
  ...
  buildMlFeatures(data) {
    const allGrades = this.getAllGradesFromDetailedFields(data);
    const passCriteria = this.thresholds.passCriteria || 60;
    const avgGrade = allGrades.length
      ? allGrades.reduce((sum, g) => sum + (Number(g.score) || 0), 0) /
        allGrades.length
      : 0;

    const failingCount = allGrades.filter(
      (g) => (Number(g.score) || 0) < passCriteria,
    ).length;
